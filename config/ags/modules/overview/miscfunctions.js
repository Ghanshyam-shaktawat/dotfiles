...
export function launchCustomCommand(command) {
  ...
  if (args[0] == ">raw") {
    // Mouse raw input
    Utils.execAsync("hyprctl -j getoption input:accel_profile").then(
      (output) => {
    ...
    execAsync([
      `bash`,
      `-c`,
      `${App.configDir}/scripts/color_generation/switchcolor.sh --pick`,
      `&`,
    ]).catch(print);
    ...
    execAsync([
      `bash`,
      `-c`,
      `mkdir -p ${GLib.get_user_state_dir()}/ags/user && sed -i "1s/.*/light/"  ${GLib.get_user_state_dir()}/ags/user/colormode.txt`,
    ])
      .then(
        execAsync([
          "bash",
    ...
    execAsync([
      `bash`,
      `-c`,
      `mkdir -p ${GLib.get_user_state_dir()}/ags/user && sed -i "1s/.*/dark/"  ${GLib.get_user_state_dir()}/ags/user/colormode.txt`,
    ])
      .then(
        execAsync([
          "bash",
    ...
    // Shut down
    execAsync([`bash`, `-c`, `systemctl poweroff || loginctl poweroff`]).catch(
      print,
    );
    ...
    // Sleep
    execAsync([`bash`, `-c`, `systemctl suspend || loginctl suspend`]).catch(
      print,
    );
...
export function execAndClose(command, terminal) {
  ...
  if (terminal) {
    execAsync([
      `bash`,
      `-c`,
      `${userOptions.apps.terminal} fish -C "${command}"`,
      `&`,
    ]).catch(print);
