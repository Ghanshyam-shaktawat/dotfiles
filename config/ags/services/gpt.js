...
class GPTService extends Service {
  ...
  _url = GLib.Uri.parse(
    PROVIDERS[this._currentProvider]["base_url"],
    GLib.UriFlags.NONE,
  );
  ...
  _initChecks() {
    ...
    if (fileExists(this._key_file_location))
      this._key = Utils.readFile(this._key_file_location).trim();
    else this.emit("hasKey", false);
    this._url = GLib.Uri.parse(
      PROVIDERS[this._currentProvider]["base_url"],
      GLib.UriFlags.NONE,
    );
  }
  ...
  get modelName() {
    return PROVIDERS[this._currentProvider]["model"];
  }
  get getKeyUrl() {
