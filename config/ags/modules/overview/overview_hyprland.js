...
const NUM_OF_WORKSPACES_SHOWN =
  userOptions.overview.numOfCols * userOptions.overview.numOfRows;
...
export default (overviewMonitor = 0) => {
  ...
  const Window = (
    ...
    const revealInfoCondition =
      Math.min(w, h) * userOptions.overview.scale > 70;
    ...
    if (x + w <= 0)
      x += Math.floor(x / monitors[monitor].width) * monitors[monitor].width;
    else if (x < 0) {
      w = x + w;
      x = 0;
    }
    if (y + h <= 0)
      x += Math.floor(y / monitors[monitor].height) * monitors[monitor].height;
    else if (y < 0) {
      h = y + h;
      y = 0;
    }
    ...
    return Widget.Button({
      attribute: {
        ...
        updateIconSize: (self) => {
          appIcon.size =
            (Math.min(self.attribute.w, self.attribute.h) *
      ...
      onMiddleClickRelease: () =>
        Hyprland.messageAsync(`dispatch closewindow address:${address}`),
      ...
      attribute: {
        ...
        getMonitorMap: (box) => {
          execAsync("hyprctl -j monitors").then((monitors) => {
            box.attribute.monitorMap = JSON.parse(monitors).reduce(
              (acc, item) => {
  ...
  return Widget.Revealer({
    ...
    child: Widget.Box({
      ...
      children: Array.from(
        { length: userOptions.overview.numOfRows },
        (_, index) =>
          OverviewRow({
