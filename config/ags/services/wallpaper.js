...
class WallpaperService extends Service {
  ...
  _save() {
    Utils.writeFile(JSON.stringify(this._wallJson), this._wallPath).catch(
      print,
    );
  }
  ...
  set(path, monitor = -1) {
    ...
    if (this._wallJson.length < this._monitorCount)
      this._wallJson[this._monitorCount - 1] = "";
    if (monitor == -1) this._wallJson.fill(path);
    else this._wallJson[monitor] = path;
  ...
  constructor() {
    ...
    try {
      ...
      Utils.writeFile("[]", this._wallPath)
        .then(() => {
