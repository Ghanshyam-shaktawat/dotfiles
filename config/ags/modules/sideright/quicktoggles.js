...
export const ToggleIconVpn = (props = {}) =>
  Widget.Button({
    className: "txt-small sidebar-iconbutton",
    ...
    onClicked: () => {
      ...
      if (vpnConnection.state === "connected") {
        ...
        vpnConnection.setConnection(true);
      }
    },
    child: MaterialIcon("vpn_lock"),
    setup: (self) => {
      setupCursorHover(self);
      self.hook(Network, (button) => {
        const vpnConnection = Network.vpn.getConnection(
        ...
        button.tooltipText =
          vpnConnection.state === "connected"
            ? `${vpnConnection.id} Connected`
            : `${vpnConnection.id} Disconnected`;
      });
    },
    ...props,
  });
        ...
        Utils.execAsync("pkill gammastep")
          .then(() => {
            ...
            const source = setInterval(() => {
              Utils.execAsync("pkill -0 gammastep").catch(() => {
                self.sensitive = true;
                source.destroy();
              });
            }, 500);
