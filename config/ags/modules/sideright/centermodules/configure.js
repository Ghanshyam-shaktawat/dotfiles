...
const HyprlandToggle = ({
  ...
  disableValue = 0,
  extraOnChange = () => {},
  checkValue = JSON.parse(exec(`hyprctl getoption -j ${option}`))["int"] != 0,
}) =>
  ConfigToggle({
    icon: icon,
    name: name,
          ...
          children: [
            ...
            Subcategory([
              HyprlandToggle({
                icon: "stack_off",
                name: "X-ray",
                desc: "Make everything behind a window/layer except the wallpaper not rendered on its blurred surface\nRecommended to improve performance (if you don't abuse transparency/blur) ",
                option: "decoration:blur:xray",
              }),
              HyprlandSpinButton({
                icon: "target",
                name: "Size",
                desc: "Adjust the blur radius. Generally doesn't affect performance\nHigher = more color spread",
              ...
              HyprlandSpinButton({
                icon: "repeat",
                name: "Passes",
                desc: "Adjust the number of runs of the blur algorithm\nMore passes = more spread and power consumption\n4 is recommended\n2- would look weird and 6+ would look lame.",
            ...
            HyprlandToggle({
              icon: "animation",
              name: "Animations",
              desc: "Enable Hyprland and GTK animations",
        ...
        ConfigSection({
          ...
          children: [
            HyprlandToggle({
              icon: "sports_esports",
              name: "Game Mode",
              desc: "Enable hyprland game mode for disable hypreffects",
              option: "decoration:rounding",
              checkValue:
                JSON.parse(exec(`hyprctl getoption -j animations:enabled`))[
                  "int"
                ] == 0,
              enableValue: 0,
              disableValue: 1,
              extraOnChange: (self, newValue) => {
                if (newValue)
                  execAsync([
                    ...
                    "keyword decoration:drop_shadows 0;",
                    "keyword general:gaps_in 0;",
                    "keyword general:gaps_out 0;",
                    "keyword general:border_size 1;",
                  ]).catch(print);
                else execAsync(["hyprctl", "reload"]).catch(print);
              },
            }),
            ConfigToggle({
              icon: "colors",
              name: "Wallbash",
              desc: "Enable Wallbash to select color from wallpaper",
              initValue: JSON.parse(
                exec(
                  `sh /home/ghanshyam/.config/hypr/scripts/GlobalControl.sh wall`,
                ),
              ),
              onChange: (self, newValue) => {
                execAsync([
                  "sh",
                  "/home/ghanshyam/.config/hypr/scripts/wallbashtoggle.sh",
                ]).catch(print);
              },
            }),
          ],
        ...
        ConfigSection({
          ...
          children: [
            HyprlandToggle({
              icon: "speed",
              name: "Show FPS",
              desc: "[Nerd] Show Hyprland's FPS overlay",
            ...
            HyprlandToggle({
              icon: "destruction",
              name: "Damage blink",
              desc: "[Epilepsy warning!] [Nerd] Show screen damage flashes",
