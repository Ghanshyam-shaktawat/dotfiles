...
const { Box, Button, Entry, Icon, Label, Revealer, Scrollable, Slider, Stack } =
  Widget;
...
const WifiNetwork = (accessPoint) => {
  const networkStrength = MaterialIcon(
    MATERIAL_SYMBOL_SIGNAL_STRENGTH[accessPoint.iconName],
    "hugerass",
  );
            ...
            updateNetworks: (self) => {
              ...
              self.children = Object.values(
                accessPoints.reduce((a, accessPoint) => {
                  ...
                  if (
                    !a[accessPoint.ssid] ||
                    a[accessPoint.ssid].strength < accessPoint.strength
                  ) {
